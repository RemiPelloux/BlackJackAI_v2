...
class Game:
    ...
    def load_player_money(self):
        try:
            with open('player_data.json', 'r') as file:
                data = json.load(file)
                return data.get('player_money', 1000)
    ...
    def check_winner(self):
        player_blackjack = self.check_blackjack(self.player.hand)
        dealer_blackjack = self.check_blackjack(self.dealer.hand)
    ...
    def resolve_bets(self, winner):
        if winner == 'player':
            self.player.adjust_money(2 * self.player.current_bet)
        elif winner == 'player_blackjack':
            self.player.adjust_money(2.5 * self.player.current_bet)
        elif winner == 'push':
            self.player.adjust_money(self.player.current_bet)
